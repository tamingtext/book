...
public class LuceneTagExtractor {
  ...
  public static void dumpTags(File file, String field, long maxDocs) throws IOException {
    ...
    Terms te = MultiFields.getTerms(reader, field);
    TermsEnum iterator = te.iterator(null);
    BytesRef next = null;
    while ((next = iterator.next()) != null) {
      System.err.printf("%s %d\n", new String(next.utf8ToString()), iterator.docFreq());
    }
  ...
  public static void emitTextForTags(File file, File output) throws IOException {
    ...
    BytesRef next = null;
    while ((next = iterator.next()) != null) {
      if (iterator.docFreq() > 30) {
        File f = new File(output, next.utf8ToString() + ".txt");
        PrintWriter pw = new PrintWriter(new FileWriter(f));
        System.err.printf("%s %d\n", next.utf8ToString(), iterator.docFreq());

        //TermDocs td = reader.termDocs(term);
        DocsEnum docsEnum = MultiFields.getTermDocsEnum(reader, null, field, next);
        int doc;
        while ((doc = docsEnum.nextDoc()) != DocsEnum.NO_MORE_DOCS) {
          buf.setLength(0);
          appendVectorTerms(buf, reader.getTermVector(doc, "description-clustering"));
          appendVectorTerms(buf, reader.getTermVector(doc, "extended-clustering"));
          emitTagDoc(next, pw, buf);
        }
  ...
  public static void emitTagDoc(BytesRef term, PrintWriter pw, StringBuilder b) {
  ...
  public static void appendVectorTerms(StringBuilder buf, Terms tv) throws IOException {
    if (tv == null) return;
    TermsEnum iterator = tv.iterator(null);
    BytesRef next = null;
    while ((next = iterator.next()) != null) {
      //System.err.printf("%s %d\n", new String(next.utf8ToString()), iterator.docFreq());
      ...
      for (int k = 0; k < freq; k++) {
        buf.append(next.utf8ToString()).append(' ');
      }
    }
  }
}
