...
public final class BayesVectorsFromLucene {

  private static final Logger log = LoggerFactory.getLogger(BayesVectorsFromLucene.class);
  ...
  public void dumpVectors() throws IOException {
    ...
    Preconditions.checkArgument(maxDFPercent >= 0 && maxDFPercent <= 100, "maxDFPercent must be between 0 and 100");
    ...
    LuceneCategoryIterable iterable;
    if (norm == LuceneIterable.NO_NORMALIZING) {
      iterable = new LuceneCategoryIterable(reader, idField, field, categoryField, termInfo, weight, LuceneIterable.NO_NORMALIZING,
          maxPercentErrorDocs);
    } else {
      iterable = new LuceneCategoryIterable(reader, idField, field, categoryField, termInfo, weight, norm, maxPercentErrorDocs);
    }
  ...
  public static void main(String[] args) throws IOException {
    ...
    try {
      ...
      if (cmdLine.hasOption(inputOpt)) { // Lucene case
        BayesVectorsFromLucene luceneDriver = new BayesVectorsFromLucene();
