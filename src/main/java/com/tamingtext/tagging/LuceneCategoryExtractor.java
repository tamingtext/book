...
public class LuceneCategoryExtractor {
  ...
  public static void dumpDocumentFields(File indexDir, String field, long maxDocs, PrintWriter out) throws IOException {
    ...
    long max = Math.min(maxDocs, reader.maxDoc());
    List<AtomicReaderContext> leaves = reader.leaves();
    for (AtomicReaderContext leaf : leaves) {
      AtomicReader aReader = leaf.reader();
      Bits liveDocs = aReader.getLiveDocs();
      for (int i = 0; i < max; i++) {
        if (liveDocs == null || liveDocs.get(i)) {//this document is not deleted
          Document d = aReader.document(i);
